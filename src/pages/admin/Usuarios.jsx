...
import { adminUsuarios } from '../../lib/api.js'

// Quem se cadastrou, e o que cada um respondeu.
//
// Assinantes e gratuitos em secoes separadas, e nao misturados com um selo: a
// pergunta que se faz aqui quase sempre e sobre um dos dois grupos, e uma
// lista unica obriga a varrer com o olho procurando o selo.
export default function Usuarios() {
  ...
  useEffect(() => {
    adminUsuarios(token).then(d => setLista(d.usuarios)).catch(e => setErro(e.message))
  }, [token])
  ...
  const assinantes = filtradas.filter(p => p.assinante)
  const gratuitos = filtradas.filter(p => !p.assinante)
  const recebido = lista.reduce((s, p) => s + (p.pago_centavos ?? 0), 0) / 100

  return (
    <div>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="subtitulo text-2xl">Usuários</h2>
          <p className="mt-1 text-sm text-grafia">
            ...
            {' · '}{recebido.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} recebido
      ...
      {q && !filtradas.length && <p className="text-sm text-grafia">Nada encontrado.</p>}

      <Grupo titulo="Assinantes" usuarios={assinantes} total={lista.filter(p => p.assinante).length}
             vazio="Ninguém assinando ainda."
             token={token} aberta={aberta} setAberta={setAberta} />

      <Grupo titulo="Gratuitos" usuarios={gratuitos} total={lista.filter(p => !p.assinante).length}
             vazio="Ninguém no gratuito."
             token={token} aberta={aberta} setAberta={setAberta} />
    </div>
  )
}

function Grupo({ titulo, usuarios, total, vazio, token, aberta, setAberta }) {
  return (
    <section className="mb-8">
      <h3 className="rotulo mb-2">
        {titulo}
        <span className="ml-2 font-normal normal-case tracking-normal tabular-nums text-tenue">
          {usuarios.length}{usuarios.length !== total && ` de ${total}`}
        </span>
      </h3>

      {!usuarios.length ? (
        <p className="cartao p-4 text-sm text-grafia">{vazio}</p>
      ) : (
          ...
          {usuarios.map(p => (
            <div key={p.id}>
              <button onClick={() => setAberta(a => a === p.id ? null : p.id)}
                ...
                {!p.confirmada && (
                  <span className="shrink-0 text-[10px] text-grafia">não confirmou email</span>
                )}
...
function Detalhe({ token, id }) {
  ...
  useEffect(() => {
    adminUsuarios(token, id).then(setDados).catch(e => setErro(e.message))
  }, [token, id])
  ...
  return (
    <div className="space-y-5 border-t border-borda bg-papel p-5">
      <div className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-3">
        ...
        <Campo r="Último acesso" v={usuario.ultimo_acesso &&
          new Date(usuario.ultimo_acesso).toLocaleString('pt-BR')} />
      </div>

      {usuario.assinatura && usuario.assinatura.status !== 'nenhuma' && (
        <p className="text-sm">
          <strong>Assinatura:</strong> {usuario.assinatura.status}
          {usuario.assinatura.period_end &&
            ` até ${new Date(usuario.assinatura.period_end).toLocaleDateString('pt-BR')}`}
          {usuario.assinatura.gateway && ` · ${usuario.assinatura.gateway}`}
        </p>
      )}

      {usuario.pagamentos?.length > 0 && (
        <div>
          <p className="rotulo mb-2">Pagamentos</p>
          <div className="cartao divide-y divide-borda">
            {usuario.pagamentos.map((p, i) => (
      ...
      {usuario.comunidades?.length > 0 && (
        <div>
          <p className="rotulo mb-2">Comunidades</p>
          <div className="cartao divide-y divide-borda">
            {usuario.comunidades.map((c, i) => (
              <div key={i} className="flex flex-wrap items-baseline gap-x-3 p-3 text-sm">
