...
function desenhar(ctx, { posicao, rotulo, familia, tagline, cor, economia, costumes, metodo }) {
  ctx.fillStyle = '#f7f5f0'; ctx.fillRect(0, 0, L, A)
  ...
  ctx.fillStyle = '#dcd7cc'; ctx.fillRect(rx, ry, rw, 16)
  const px = rx + (posicao / 100) * rw
  ctx.fillStyle = cor
  ctx.beginPath(); ctx.arc(px, ry + 8, 22, 0, Math.PI * 2); ctx.fill()
  ctx.strokeStyle = '#f7f5f0'; ctx.lineWidth = 6; ctx.stroke()

  ctx.font = '400 24px system-ui, sans-serif'; ctx.fillStyle = '#5b5f6b'
  ...
  // A familia politica.
  ctx.fillStyle = '#12141a'; ctx.font = '400 76px Georgia, serif'
  ctx.fillText(familia, 90, 660)

  ctx.fillStyle = '#5b5f6b'; ctx.font = '400 34px system-ui, sans-serif'
  ...
  if (metodo) {
    ...
    ctx.fillStyle = '#12141a'; ctx.font = '600 30px system-ui, sans-serif'
  ...
  ctx.fillStyle = '#5b5f6b'; ctx.font = '400 28px system-ui, sans-serif'
...
function barra(ctx, titulo, valor, x, y, cor) {
  ...
  ctx.fillStyle = '#5b5f6b'; ctx.font = '400 30px system-ui, sans-serif'
  const n = String(valor); ctx.fillText(n, x + w - ctx.measureText(n).width, y)
  ctx.fillStyle = '#dcd7cc'; ctx.fillRect(x, y + 22, w, 12)
  ctx.fillStyle = cor
...
export default function Compartilhar({ posicao, familia, cor }) {
  ...
  async function gerar() {
    ...
    // No celular, compartilhar direto abre o Instagram na lista. No desktop
    // nao existe, entao cai para download.
    if (navigator.canShare?.({ files: [arquivo] })) {
      try { await navigator.share({ files: [arquivo], title: 'Eu Presidente' }) }
  ...
  return (
    <div className="cartao p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={gerar} disabled={gerando} className="botao-forte">
          {gerando ? 'Gerando...' : 'Gerar imagem'}
        </button>
        <button onClick={copiarLink} className="botao-leve">
          {copiado ? 'Link copiado' : 'Copiar link do resultado'}
        </button>
      </div>
      <p className="mt-3 text-xs leading-relaxed text-grafia">
        Quem abrir o link vê seu resultado inteiro — o link é a credencial. Mande para quem
        você quer que veja.
      </p>
